...
public sealed class LinqExercises
{
    ...
    public IEnumerable<string> Task11_JoinStudentsWithEnrollments()
    {
        return UniversityData.Students
            .Join(
                ...
                s => s.Id,
                e => e.StudentId,
                (s, e) => $"{s.FirstName} {s.LastName} | Enrollment Date: {e.EnrollmentDate:yyyy-MM-dd}"
            );
    }
    ...
    public IEnumerable<string> Task14_AverageGradePerCourse()
    {
        return UniversityData.Enrollments
            ...
            .GroupBy(x => x.Title)
            .Select(g => $"{g.Key} | Avg grade: {g.Average(x => x.FinalGrade!.Value):F2}");
    }
    ...
    public IEnumerable<string> Task16_HighestGradePerStudent()
    {
        return UniversityData.Students
            .Join(
                ...
                s => s.Id,
                e => e.StudentId,
                (s, e) => new { Student = s, e.FinalGrade }
            )
            .GroupBy(x => new { x.Student.FirstName, x.Student.LastName })
            .Select(g => $"{g.Key.FirstName} {g.Key.LastName} | Max grade: {g.Max(x => x.FinalGrade!.Value):F2}");
    }
    ...
    public IEnumerable<string> Challenge02_AprilCoursesWithoutFinalGrades()
    {
        return UniversityData.Courses
            ...
            .Join(
                ...
                c => c.Id,
                e => e.CourseId,
                (c, e) => new { Course = c, e.FinalGrade }
            )
            .GroupBy(x => x.Course.Title)
            .Where(g => g.All(x => !x.FinalGrade.HasValue))
            .Select(g => g.Key);
    }
    ...
    public IEnumerable<string> Challenge03_LecturersAndAverageGradeAcrossTheirCourses()
    {
        return UniversityData.Lecturers
            ...
            .Join(
                ...
                lc => lc.Course.Id,
                e => e.CourseId,
                (lc, e) => new { lc.Lecturer, e.FinalGrade }
            )
            .GroupBy(x => new { x.Lecturer.FirstName, x.Lecturer.LastName })
